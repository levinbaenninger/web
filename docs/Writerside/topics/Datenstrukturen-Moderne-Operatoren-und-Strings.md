# Datenstrukturen, Moderne Operatoren und Strings

## Inhaltsverzeichnis - Datenstrukturen, Moderne Operatoren und Strings

In diesem Kapitel werden folgende Themen behandelt:

### Moderne Operatoren

- [Destructuring Arrays](Destructuring-Arrays.md)
- [Destructuring Objects](Destructuring-Objects.md)
- [Der Spread Operator](Spread.md)
- [Rest Pattern und Parameter](Rest-Pattern-und-Parameter.md)
- [Kurzschlüsse (`&&` und `||`)](Kurzschlüsse-und.md)
- [Der Nullish Coalescing Operator (??)](Der-Nullish-Coalescing-Operator.md)
- [Die logischen Zuweisungsoperatoren](Die-Logischen-Zuweisungsoperatoren.md)

### Moderne Features

- [`for .. of`-Loop](For-Of-Loop.md)
- [Object Literals - Verbesserungen](Object-Literals-Verbesserungen.md)
- [Optional Chaining (`?.`)](Optional-Chaining.md)
- [Über Objekte loopen](Über-Objekte-loopen.md)

### Datenstrukturen

- [Sets](Sets.md)
- [Maps](Maps.md)

### Strings

- [Strings - Grundlagen](Strings-Grundlagen.md)
- [Strings - Wichtige Methoden](Strings-Wichtige-Methoden.md)
- [Strings - Weitere Methoden](Strings-Weitere-Methoden.md)
